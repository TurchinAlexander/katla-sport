...
namespace KatlaSport.Services.HiveManagement
{
    ...
    public class HiveSectionService : IHiveSectionService
    {
        ...
        {
            var dbHiveSections = await _context.Sections.Where(hs => hs.Code == createRequest.Code).ToArrayAsync();
            if (dbHiveSections.Length > 0)
            {
        ...
        {
            var dbHiveSections = await _context.Sections.Where(hs => hs.Id == hiveSectionId).ToArrayAsync();
            if (dbHiveSections.Length == 0)
            {
                throw new RequestedResourceNotFoundException();
            }
